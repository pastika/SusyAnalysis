...
public:
  ...
  // 
  //*** DCH add for Sel Tagger
  void set_SelTag(bool selTag) {_useSelTag = selTag;}
  //*** End DCH add
...
private:
  ...
  //***** DCH edit for Selected HEPTopTagger
  bool _useSelTag;
  //*** end DCH edit
...
HEPTopTagger::HEPTopTagger(const fastjet::ClusterSequence & cs,
  ...
  _m23cut(0.35),_m13cutmin(0.2),_m13cutmax(1.3),
  _nfilt(5),_jet_algorithm(cambridge_algorithm),_jet_algorithm_recluster(kt_algorithm),
  debugg(false),_useSelTag(false)
{}
...
void HEPTopTagger::run_tagger()
{
  ...
  for(unsigned rr=0; rr<_top_parts.size(); rr++){
	...
	//******* DCH Edit for Selected HEP Top Tagger
	if(_useSelTag) {
	  if((deltatop < _delta_top) && !_is_masscut_passed) {
	    _delta_top = deltatop;
	    _is_maybe_top = true;
	  ...
	  else if((deltatop < _delta_top) && _is_masscut_passed && check_mass_criteria(top_subs)) {
	    _delta_top = deltatop;
	    _is_maybe_top = true;
	    _top_candidate = topcandidate;
	    store_topsubjets(top_subs);
