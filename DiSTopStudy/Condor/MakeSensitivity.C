...
void MakeSensitivity(){
  ...
  MakeIndividualPlots(15);
  iFilesArray[iFilesArrayCount]=15;
  iFilesArrayCount++;
  ...
  */
  ...
  for(Int_t i=0;i<iFilesArrayCount;i++){
    ...
    DQHtml<<"<TD  align='left' colspan=1 >"<<hTemp1->Integral("weight")<<"</TD>"<< endl;
    TH1F *hTemp2 = (TH1F*)(fIn->Get("TDist_Signal_Leading_Mass"));
    DQHtml<<"<TD  align='left' colspan=1>"<<hTemp2->Integral("weight")<<"</TD>"<< endl;
    DQHtml<<"</TR>"<< endl;
  }
